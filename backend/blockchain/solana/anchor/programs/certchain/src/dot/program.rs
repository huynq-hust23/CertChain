...
impl<'info, 'entrypoint> Certificate {
    pub fn load(
        ...
        let original_data_sha256 = Mutable::new(account.original_data_sha256.clone());

        let original_image_sha256 = Mutable::new(account.original_image_sha256.clone());
    ...
    pub fn store(loaded: Mutable<LoadedCertificate>) {
        ...
        let original_data_sha256 = loaded.original_data_sha256.borrow().clone();

        loaded.__account__.original_data_sha256 = original_data_sha256;

        let original_image_sha256 = loaded.original_image_sha256.borrow().clone();
...
pub fn init_certificate_handler<'info>(
    ...
    assign!(
        cert.borrow_mut().delivery_date,
        Mutable::<[u8; 4]>::new(delivery_date)
    );

    assign!(
        cert.borrow_mut().serial_id,
        Mutable::<[u16; 64]>::new(serial_id)
    );

    assign!(
        cert.borrow_mut().security_code,
        Mutable::<[u8; 32]>::new(security_code)
    );

    assign!(
        cert.borrow_mut().more_info,
        Mutable::<[u16; 256]>::new(more_info)
    );

    assign!(
        cert.borrow_mut().original_data_sha256,
        Mutable::<[u8; 32]>::new(original_data_sha256)
    );

    assign!(
        cert.borrow_mut().original_image_sha256,
        Mutable::<[u8; 32]>::new(original_image_sha256)
    );
}
